...
async function startHisoka() {
  ...
  const client = XeonBotIncConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["Bot By 🦄Xeon", "Safari", "5.1.7"],
    auth: state,
  });
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      require("./UnicornXeon")(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        ...
        console.log(`Device Logged Out, Please Delete Folder Session session and Scan Again.`);
        process.exit();
      ...
      console.log(color("Developer Number: +916909137213", "yellow"));
